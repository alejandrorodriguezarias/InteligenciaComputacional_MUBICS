...
function [Recall,Spec,Precision,NPV,ACC,F1Score, prediction] = trainingDiscr(typeDiscr, cv, inputs, outputs)
...
for i = 1:cv.NumTestSets
    ...
    prediction{i} = predict(mdlListDiscrLinear{i}, inputs(teIdx,:));
    [CM, orderCM] = confusionmat(outputs(teIdx,:), prediction{i});
    for j = 1:size(CM,1)
